...
const TZS: { key: string; tz: string; label: string }[] = [
...
export default function TimezoneBar() {
  ...
  async function switchTz(tz: string) {
    await api("/api/profile", { method: "PATCH", body: { timezone: tz } });
    await refreshMe();
    await refreshStatus();
  }
  return (
    <div className="panel mx-2 mt-2 p-2 grid grid-cols-4 gap-1">
      {TZS.map((t) => (
        <button
          key={t.key}
          onClick={() => switchTz(t.tz)}
          className={`pixel-btn !py-1 !px-2 text-xs ${active === t.tz ? "primary" : ""}`}
        >
          {t.label}
        </button>
      ))}
